...
class WordbookEntry(Base):
    ...
    provider_code: Mapped[Optional[str]] = mapped_column(String(25), nullable=True)
    ...
    color: Mapped[Optional[str]] = mapped_column(String(15), nullable=True)
    group_id: Mapped[Optional[int]] = mapped_column(ForeignKey("word_groups.id", ondelete="SET NULL"), nullable=True, index=True)
    created_at: Mapped[datetime] = mapped_column(server_default=func.now())
    updated_at: Mapped[datetime] = mapped_column(server_default=func.now(), onupdate=func.now())

    user: Mapped["User"] = relationship(back_populates="wordbookEntries")
    group: Mapped[Optional["WordGroup"]] = relationship(back_populates="entries")
